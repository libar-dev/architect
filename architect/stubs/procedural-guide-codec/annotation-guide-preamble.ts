 ...
 * `@architect` opt-in marker. Files without this marker are invisible
 * to the scanner.
 *
 * ### File-Level Opt-In
 *
 ...
 * | Symptom | Cause | Fix |
 * |---------|-------|-----|
 * | Pattern not in scanner output | Missing `@architect` opt-in | Add file-level `@architect` JSDoc/tag |
