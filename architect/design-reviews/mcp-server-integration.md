...
**Source:** `architect/specs/mcp-server-integration.feature`
...
sequenceDiagram
    ...
    Note over mcp_server: Rule 5 — The server works with .mcp.json (Claude Code), claude_desktop_config.json (Claude Desktop), and any MCP client. It accepts —input, —features, —base-dir args, auto-detects architect.config.ts, and reports the package version accurately through the CLI.
