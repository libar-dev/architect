@architect
@architect-pattern:StatusAwareEslintSuppression
@architect-status:roadmap
@architect-phase:99
@architect-effort:2d
@architect-product-area:Validation
@architect-executable-specs:tests/features/behavior/status-aware-eslint
Feature: Status-Aware ESLint Suppression - Annotation-Driven Lint Rule Relaxation

  **Problem:**
  Design artifacts (code stubs with `@architect-status roadmap`) intentionally have unused
  ...
  **Solution:**
  ...
  1. Reads `@architect-status` from file-level JSDoc comments
  ...
  **Why It Matters:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | ESLint processor plugin | pending | @libar-dev/architect/src/eslint/processor.ts | Yes | unit |
      | Status-to-config mapper | pending | @libar-dev/architect/src/eslint/status-config.ts | Yes | unit |
      | ESLint plugin entry point | pending | @libar-dev/architect/src/eslint/index.ts | Yes | unit |
      | CLI command for ignore list | pending | @libar-dev/architect/src/cli/eslint-ignores.ts | Yes | unit |
  ...
  Rule: File status determines unused-vars enforcement

    **Invariant:** Files with `@architect-status roadmap` or `deferred` have relaxed
    ...
    **Verified by:** Roadmap file has relaxed rules, Completed file has strict rules, No status file has strict rules
    ...
    Scenario: Roadmap file has relaxed unused-vars rules
    ...
    @acceptance-criteria @happy-path
    Scenario: Completed file has strict unused-vars rules
      Given a TypeScript file with JSDoc containing:
        """typescript
        /**
         * @architect
    ...
    Scenario: File without status tag has strict rules
      Given a TypeScript file without any @architect-status tag
      When ESLint processes the file with the status-aware processor
      Then unused exports ARE reported as errors
      And the default strict configuration applies

  # ===========================================================================
  # RULE 2: Reuse Process Guard infrastructure
  # ===========================================================================

  Rule: Reuses deriveProcessState for status extraction
    ...
    **Verified by:** Protection level from Process Guard, Consistent status mapping
    ...
    @acceptance-criteria @happy-path
    Scenario: Protection level matches Process Guard derivation
      Given a file with @architect-status:roadmap
      When Process Guard derives protection level
      And ESLint processor derives protection level
      Then both return "none"

    @acceptance-criteria @happy-path
    Scenario: Status-to-protection mapping is consistent
  ...
  # ===========================================================================
  # RULE 3: ESLint Processor implementation
  # ===========================================================================

  Rule: ESLint Processor filters messages based on status
    ...
    **Verified by:** Processor filters in postprocess, No source modification
    ...
    @acceptance-criteria @happy-path
    Scenario: Processor filters messages in postprocess
    ...
    @acceptance-criteria @validation
    Scenario: No source code modification occurs
      Given a TypeScript file with @architect-status:roadmap
      When the processor runs
      Then file content on disk is unchanged
      And no eslint-disable comments are present in the file

    @acceptance-criteria @happy-path
    Scenario: Non-relaxed rules pass through unchanged
  ...
  # ===========================================================================
  # RULE 4: CLI integration for static configuration
  # ===========================================================================

  Rule: CLI can generate static ESLint ignore list
    ...
    **Verified by:** CLI generates file list, List includes only relaxed files
    ...
    @acceptance-criteria @happy-path
    Scenario: CLI generates ESLint ignore file list
    ...
    @acceptance-criteria @happy-path
    Scenario: JSON output mode for programmatic consumption
      When running "pnpm lint:process --eslint-ignores --json"
      Then output is valid JSON
      And JSON contains array of file paths with protection level "none"

  # ===========================================================================
  # RULE 5: Migration from directory-based exclusions
  # ===========================================================================

  Rule: Replaces directory-based ESLint exclusions

    **Invariant:** After implementation, the directory-based exclusions in eslint.config.js
    (lines 30-57) are removed. All suppression is driven by @architect-status annotations.
    ...
    **Target State:**
    """javascript
    // eslint.config.js
    import { statusAwareProcessor } from "@libar-dev/architect/eslint";
    ...
    **Verified by:** Directory exclusions removed, Processor integration added
    ...
    @acceptance-criteria @happy-path
    Scenario: Directory exclusions are removed after migration
    ...
    Scenario: Existing roadmap files still pass lint
      Given roadmap files that previously relied on directory exclusions:
        | File |
        | architect/stubs/reservation-pattern/reservation-pattern.ts |
        | architect/stubs/durability-types/durability-types.ts |
      When running "pnpm lint" after migration
      Then files pass lint (no unused-vars errors)
      And files have @architect-status:roadmap annotations

  # ===========================================================================
  # RULE 6: Configurable rule relaxation
  # ===========================================================================

  Rule: Rule relaxation is configurable
    ...
    **Verified by:** Default rules are relaxed, Custom rules can be configured
    ...
    @acceptance-criteria @happy-path
    Scenario: Default configuration relaxes no-unused-vars
    ...
    @acceptance-criteria @happy-path
    Scenario: Custom rules can be configured for relaxation
