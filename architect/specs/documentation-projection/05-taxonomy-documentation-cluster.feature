...
Feature: TaxonomyDocumentationCluster - the MVP proof-point: one taxonomy source, many audience-shaped documents
  ...
  **The cluster (one source → many shapes):** source = the tag registry (`architect-core`), reused through the shipped `TaxonomyDigestProjection` (`projectTaxonomyDigest`); the cluster never calls the registry builder directly, it reads the digest. Targets:
  ...
  - the live-API taxonomy context served to the API/MCP sink (the `architect_taxonomy` tool).
  ...
  **Sequencing & prerequisites:** the cluster ships in a No-BC-safe order. Steps 1–4 **shipped this campaign**; only step 5 (`GoalOrientedNavigation`'s registry re-home) remains, and it is a later, separately-owned pattern — this cluster is its prerequisite, not its dependency.
  ...
  **Open Questions:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Live-API taxonomy context | complete | no descriptor (API sink) | `architect_taxonomy` (MCP) |
  ...
  Rule: The taxonomy documents are one generation family from the tag registry
    ...
    **Verified by:** `docs-live/TAXONOMY.md` regenerates from `projectTaxonomyDigest` under the determinism gate; the live `architect_taxonomy` MCP tool emits the same tag set and counts.
