...
@architect-executable-specs:packages/architect-projection/tests/features/projections/documentation-composition/taxonomy-documentation-cluster.feature
  ...
  **Open Questions:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Skill shape (model + link-to-live) | complete | embedded-region (markdown-file) | .agents/skills/architect-base/references/taxonomy.md (`taxonomy-role-enum` + `taxonomy-tag-count` regions, `taxonomy-skill` generator) |
      | Formal-spec shape (enumeration in normative prose) | deferred | embedded-region (markdown-file) | formal-spec/04-tag-registry.md — design resolved per epic 2026-06-05, pending one proof-slice implement (modality is a projected source fact; the RFC function grouping is an audience-shaped View read; projecting modality dissolves the column-span blocker). Per-group table rendering + N-regions-per-host capability is built and tested; remaining work is wiring one function group (`Classification`) end-to-end as the proof slice — an implement session, not a design call. |
      | Emission descriptor (BundleRouting split) | complete | n/a (contract) | packages/architect-projection/src/fragments/emission-descriptor.ts |
      | Managed-region engine (marker scan + rewrite + normalization) | complete | n/a (infrastructure) | packages/architect-projection/src/renderers/managed-region.ts (pure; loud on malformed/missing/duplicate/nested markers) |
      | Multi-target write path | complete | n/a (infrastructure) | `architect-cli`'s `cli/generate-docs.ts` embedded-generator track: reads the host, applies regions via the managed-region engine, writes the host outside the single output dir; re-checks repo containment after resolution |
      | Region-aware determinism gate | complete | n/a (infrastructure) | `reportDriftAndExit` (`architect-cli`'s `cli/generate-docs.ts`) diffs each embedded host's regenerated regions against the on-disk host (region-scoped because only inter-marker spans change); closes the docs-live-only coverage hole |
  ...
  Rule: Descriptor paths stay repo-contained and covered by the determinism gate
    ...
    **Verified by:** any descriptor path outside the repo is rejected before writing, the determinism gate reaches an embedded region outside docs-live, a drifted region in an out-of-tree host fails the gate, an embedded host that has not yet been region-prepared fails loudly rather than writing silently
    ...
    @acceptance-criteria @error
    Scenario: a present host that has not been region-prepared fails loudly rather than writing silently
      Given a configured embedded-region target whose host file exists but carries no begin/end markers for the routed regionId
      When the projection attempts to write that region
    ...
    @acceptance-criteria @boundary
    Scenario: an embedded target whose host is absent in this project is skipped, not failed
      Given a configured embedded-region target whose host file does not exist in the current base directory
      When generation runs `--all`
      Then that embedded generator is skipped with a notice (an embedded shape targets a repo-specific authored file, so an absent host means "not applicable here")
      And the run still succeeds for every other generator, so `--all` stays portable across projects
