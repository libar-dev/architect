...
Feature: OneSourceMultipleAudiences - one source materializes into audience-shaped read models
  ...
  **Resolved (per the taxonomy skill shape — born-accepted after the build, the ADR-010 pattern; the taxonomy skill is the family that exercised these. Re-open per future audience shape — the data-api skill, the MCP tool list — if it needs a different budget rule than the taxonomy skill established):**
