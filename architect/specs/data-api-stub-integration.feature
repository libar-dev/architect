...
@architect-unlock-reason:Implementation-complete-all-deliverables-done
@architect-phase:25a
@architect-product-area:DataAPI
@architect-effort:2d
@architect-priority:high
@architect-business-value:unlock-design-session-stub-metadata
Feature: Data API Stub Integration - Unlocking Design Session Data
  ...
  **Business Value:**
  ...
  Rule: All stubs are visible to the scanner pipeline
    ...
    **Invariant:** Every stub file in `architect/stubs/` has `@architect`
    opt-in and `@architect-implements` linking it to its parent pattern.

    **Rationale:** The scanner requires `@architect` opt-in marker to include a
    file. Without it, stubs are invisible regardless of other annotations. The
    `@architect-implements` tag creates the bidirectional link: spec defines the
  ...
  Rule: Stubs subcommand lists design stubs with implementation status
    ...
    **Rationale:** Before implementation, agents need to know: which stubs
    ...
    `@architect-target` paths against actual files to determine status.

    **Output per stub:**
    | Field | Source |
    | Stub file | Pattern filePath |
    | Target | @architect-target value |
    | Implemented? | Target file exists? |
    | Since | @architect-since (design session ID) |
    | Pattern | @architect-implements value |
