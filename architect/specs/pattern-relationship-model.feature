...
@architect-executable-specs:tests/features/behavior/pattern-relationships
  ...
  **Business Value:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Implements tag in taxonomy registry | complete | src/taxonomy/registry-builder.ts | Yes | unit |
      ...
      | Linter rules for relationship validation | complete | src/lint/rules.ts | Yes | unit |
  ...
  Rule: Code files declare pattern realization via implements tag
    ...
    **Invariant:** Files with `@architect-implements:PatternName,OtherPattern` are linked
    ...
    **Verified by:** Implements tag parsed, Multiple patterns supported, No conflict with pattern definition, Multiple implementations of same pattern
    ...
    Scenario: Implements tag parsed from TypeScript
      Given a TypeScript file with annotations:
         ...
         * @architect-uses idempotentAppend, Workpool
         */
        """
    ...
    Scenario: No conflict with pattern definition
      Given a roadmap spec with `@architect-pattern:EventStoreDurability`
      And a TypeScript file with `@architect-implements:EventStoreDurability`
    ...
    Scenario: Multiple files implement same pattern
      Given three TypeScript files each with `@architect-implements:EventStoreDurability`
  ...
  Rule: Pattern inheritance uses extends relationship tag
    ...
    **Verified by:** Extends tag parsed, Extended-by computed, Inheritance chain validated
    ...
    Scenario: Extended-by reverse lookup computed
      Given pattern A with `@architect-extends:B`
    ...
    Scenario: Circular inheritance detected
      Given pattern A with `@architect-extends:B`
      And pattern B with `@architect-extends:A`
  ...
  Rule: Technical dependencies use directed relationship tags
    ...
    **Invariant:** `@architect-uses` declares outbound dependencies (what this
    pattern depends on). `@architect-used-by` declares inbound dependencies
    ...
    **Verified by:** Uses rendered as solid arrows, Used-by aggregated correctly
    ...
    Scenario: Uses rendered as solid arrows in graph
      Given a pattern with `@architect-uses:CommandBus,EventStore`
    ...
    Scenario: Used-by aggregated in pattern detail
      Given pattern A with `@architect-used-by:B,C`
  ...
  Rule: Roadmap sequencing uses ordering relationship tags
    ...
    **Invariant:** `@architect-depends-on` declares what must be completed first
    (roadmap sequencing). `@architect-enables` declares what this unlocks when
  ...
  Rule: Cross-tier linking uses traceability tags (PDR-007)
    ...
    **Invariant:** `@architect-executable-specs` on roadmap specs points to test
    locations. `@architect-roadmap-spec` on package specs points back to the
    ...
    **Verified by:** Bidirectional links established, Orphan detection
    ...
    Scenario: Bidirectional links established
      Given a roadmap spec with `@architect-executable-specs:platform-core/tests/features/durability`
      And a package spec with `@architect-roadmap-spec:EventStoreDurability`
    ...
    Scenario: Orphan executable spec detected
      Given a package spec with `@architect-roadmap-spec:NonExistentPattern`
      When the linter runs
  ...
  Rule: Epic/Phase/Task hierarchy uses parent-child relationships
    ...
    **Invariant:** `@architect-level` declares the hierarchy tier (epic, phase, task).
    `@architect-parent` links to the containing pattern. This enables rollup
    progress tracking.
    ...
    **Verified by:** Parent link validated, Progress rollup calculated
    ...
    Scenario: Parent link validated
      Given a phase spec with `@architect-parent:ProcessEnhancements`
      And an epic spec with `@architect-pattern:ProcessEnhancements`
  ...
  Rule: Linter detects relationship violations
    ...
    **Verified by:** Missing target detected, Pattern conflict detected, Asymmetric link detected
    ...
    Scenario: Missing relationship target detected
      Given a file with `@architect-uses:NonExistentPattern`
    ...
    Scenario: Pattern tag in implements file causes error
      Given a file with both `@architect-implements:X` and `@architect-pattern:X`
      When the linter runs
    ...
    Scenario: Asymmetric traceability detected
      Given a roadmap spec with `@architect-executable-specs:path/to/tests`
      And no package spec at that path with `@architect-roadmap-spec` back-link
      When the linter runs with strict mode
