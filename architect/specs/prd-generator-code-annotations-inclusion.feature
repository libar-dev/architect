@architect
@architect-pattern:PrdImplementationSection
@architect-status:roadmap
@architect-phase:99
@architect-effort:3d
@architect-product-area:Generation
@architect-parent:PatternRelationshipModel
  ...
  **Business Value:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Implementation collector | pending | architect/src/generators/ | Yes | unit |
      | PRD section renderer | pending | architect/src/renderable/ | Yes | unit |
  ...
  Rule: PRD generator discovers implementations from relationship index
    ...
    **Rationale:** The `@architect-implements` tag creates a backward link from
    ...
    **Verified by:** Implementations discovered, Multiple files aggregated
    ...
    Scenario: Implementations discovered from relationship index
      ...
      And three TypeScript files with `@architect-implements:EventStoreDurability`
  ...
  Rule: Patterns without implementations render cleanly
    ...
    **Invariant:** If no files have `@architect-implements:X` for pattern X,
    ...
    **Verified by:** Section omitted when empty
    ...
    Scenario: Section omitted when no implementations exist
      ...
      And no files have `@architect-implements:FuturePattern`
      When the PRD generator runs
