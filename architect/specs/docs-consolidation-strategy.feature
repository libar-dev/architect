@architect
@architect-pattern:DocsConsolidationStrategy
@architect-status:completed
@architect-unlock-reason:Retroactive-completion
@architect-phase:35
@architect-effort:4w
@architect-product-area:Generation
@architect-depends-on:CodecDrivenReferenceGeneration
@architect-business-value:right-size-all-14-manual-docs-via-generation-relocation-and-audience-alignment
  ...
  **Scope:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Phase 2 - Codec listings extraction | complete | architect.config.ts, src/renderable/codecs/*.ts | Yes | integration |
      ...
      | Phase 37 - docs-live/ directory consolidation | complete | architect.config.ts | Yes | integration |
      | Phase 38 - Generated doc quality improvements | complete | src/renderable/codecs/reference.ts | Yes | integration |
      | Phase 39 - Session workflow CLAUDE.md module generation | complete | architect/specs/, _claude-md/workflow/ | No | n/a |
  ...
  Rule: Convention tags are the primary consolidation mechanism
    ...
    **Invariant:** Each consolidation phase follows the same pattern: register a
    ...
    `@architect-convention` tags using structured JSDoc, add a `ReferenceDocConfig`
    entry in `architect.config.ts`, and replace the manual doc section with a
