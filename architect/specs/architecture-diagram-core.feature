...
@architect-executable-specs:tests/features/behavior/architecture-diagrams
Feature: Architecture Diagram Generation - Core
  ...
  **Solution:** Generate architecture diagrams automatically from source code annotations
  ...
  - `@architect-arch-role` - Component type (preset-configurable: service, handler, repository, etc.)
  - `@architect-arch-context` - Bounded context for subgraph grouping
  - `@architect-arch-layer` - Architectural layer (domain, application, infrastructure)
