...
Feature: ADR-003 - Source-First Pattern Architecture
  ...
  **Consequences:**
  ...
  Rule: TypeScript source owns pattern identity
    ...
    **Invariant:** A pattern is defined by `@architect-pattern` in a TypeScript
  ...
  Rule: Implements is UML Realization (many-to-one)
    ...
    **Invariant:** `@architect-implements` declares a realization relationship.
  ...
  Rule: Reverse links preferred over forward links
    ...
    **Invariant:** `@architect-implements` (reverse: "I verify this pattern")
    is the primary traceability mechanism. `@architect-executable-specs`
    ...
    **Verified by:** TypeScript source is canonical pattern definition
    ...
    Reverse links are self-maintaining because the test file cannot run
    without the implementation it references. Forward links require tier 1
    spec maintenance and go stale when specs are archived.
  ...
  Scenario: TypeScript source is canonical pattern definition
    Given a pattern defined in a TypeScript file with @architect-pattern
    When an executable spec references it with @architect-implements
