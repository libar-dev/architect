...
- **Expect incompleteness.** Partial/unbuilt functionality and un-wired top-down design are the plan at this phase — do not flag them as defects. Flag deviations from *this direction*, and flag dead context that should have been deleted (a half-done model that doesn't yet compile is the expected mid-state, not a problem to paper over with an adapter).
