...
export async function deriveProcessState(config: DeriveStateConfig): Promise<Result<ProcessState>> {
  ...
  if (activeSession !== undefined) {
    (processState as { activeSession?: SessionState }).activeSession = activeSession;
  }
 ...
 */
async function deriveFileStates(
  baseDir: string,
  patterns: readonly string[]
): Promise<Result<Map<string, FileState>>> {
  // Scan feature files
  ...
  for (const file of files) {
    ...
    if (unlockInfo.unlockReason !== undefined) {
      (fileState as { unlockReason?: string }).unlockReason = unlockInfo.unlockReason;
    }
  ...
  for (const tag of tags) {
    ...
    if (tag.includes('status:')) {
      ...
      if (match?.[1]) {
        ...
        if (PROCESS_STATUS_VALUES.includes(status as ProcessStatusValue)) {
          return status as ProcessStatusValue;
        }
      }
    }
  }
 ...
 */
function extractUnlockReason(tags: readonly string[]): {
 ...
 */
function extractExcludedSpecs(_background: unknown): readonly string[] {
 ...
 */
export function isInSessionScope(state: ProcessState, relativePath: string): boolean {
 ...
 */
export function isSessionExcluded(state: ProcessState, relativePath: string): boolean {
  if (!state.activeSession) {
    return false;
  }
