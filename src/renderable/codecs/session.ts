 ...
 */
export const DEFAULT_SESSION_OPTIONS: Required<SessionCodecOptions> = {
 ...
 */
export const DEFAULT_REMAINING_WORK_OPTIONS: Required<RemainingWorkCodecOptions> = {
  ...DEFAULT_BASE_OPTIONS,
...
function buildSessionStatus(dataset: MasterDataset): SectionBlock[] {
  ...
  const currentPhase = dataset.byPhase.find(
 ...
 */
function buildSessionPatternList(
...
function buildRemainingWorkDocument(
  ...
  let incomplete: ExtractedPattern[] = [];
...
function buildRemainingByPrioritySummary(
  ...
  const unblocked: ExtractedPattern[] = [];
  const blocked: ExtractedPattern[] = [];
 ...
 */
function buildRemainingPatternTable(patterns: ExtractedPattern[]): SectionBlock[] {
  const rows = sortByPhaseAndName([...patterns]).map((p) => {
