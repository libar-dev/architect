 ...
 */
function sectionToBlock(section: ExtractedSection): ContentBlock {
  return {
 ...
 */
export function findDuplicates(blocks: ContentBlock[]): Map<string, ContentBlock[]> {
  ...
  const duplicates = new Map<string, ContentBlock[]>();
 ...
 */
function findHeaderConflicts(sections: ExtractedSection[]): Map<string, ExtractedSection[]> {
  const byHeader = new Map<string, ExtractedSection[]>();
  ...
  const conflicts = new Map<string, ExtractedSection[]>();
 ...
 */
function disambiguateHeader(section: ExtractedSection): string {
...
export function deduplicateSections(
    ...
    for (const block of duplicateBlocks) {
      if (block !== winner) {
        ...
        const idx = sections.findIndex(
          (s) => s.sourceFile === block.source && s.section === block.header
        );
        if (idx !== -1) {
  ...
  let filteredSections: ExtractedSection[] = sections
  ...
  for (const [header, conflictingSections] of headerConflicts) {
    ...
    for (const section of conflictingSections) {
      const idx = filteredSections.findIndex(
        (s) => s.section === header && s.sourceFile === section.sourceFile
      );
