...
import { ArtefactSetSchema, type ArtefactSet } from '../validation-schemas/artefact-set.js';
 ...
 */
function getCatalogueDir(): string {
 ...
 * ```
 */
export async function loadArtefactSet(
  name: string
): Promise<Result<ArtefactSet, ArtefactSetLoadError>> {
  ...
  if (!parseResult.ok) {
    const error: ArtefactSetLoadError = {
      type: 'artefact-set-load-error',
      name,
      path: filePath,
 ...
 * ```
 */
export async function listAvailableArtefactSets(): Promise<string[]> {
  const catalogueDir = getCatalogueDir();
 ...
 * ```
 */
export function formatArtefactSetError(error: ArtefactSetLoadError): string {
