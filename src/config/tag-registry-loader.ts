...
export type TagRegistryResult =
  | { ok: true; value: TagRegistry }
  | { ok: false; error: TagRegistryLoadError };
...
export async function loadTagRegistry(
  ...
  if (!parseResult.ok) {
    const error: TagRegistryLoadError = {
      type: "tag-registry-load-error",
      path: absolutePath,
 ...
 * ```
 */
export function formatTagRegistryError(error: TagRegistryLoadError): string {
