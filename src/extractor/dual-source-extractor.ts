 ...
 * ```
 */
export function extractDeliverables(feature: ScannedGherkinFile): readonly Deliverable[] {
...
export function combineSources(
  ...
  for (const [patternName, codePatternArray] of codeIndex.entries()) {
    ...
    const dualSourcePattern: DualSourcePattern = {
    ...
    if (hasCollision) {
      warnings.push(
        `Pattern name collision: "${patternName}" defined in ${codePatternArray.length} files: ` +
