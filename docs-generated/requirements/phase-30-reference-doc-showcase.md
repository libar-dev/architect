...
- Remove 120-char rule description truncation (pending)
- Deep behavior rendering with parsed invariant/rationale (pending)
- JSDoc prose in shape sections at standard level (pending)
- archLayer filter in DiagramScope (pending)
...
- List block usage for scenario names under rules (pending)
- Collapsible block for progressive disclosure (pending)
- Link-out block for cross-references (pending)
- Function signature surfacing in ExportInfo (pending)
- Param/returns/throws extraction from JSDoc (pending)
- Full property-level JSDoc without truncation (pending)
- Auto-shape discovery mode (pending)
