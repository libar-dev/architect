...
import {
  applyOutputPipeline,
  applyListFilters,
  checkOutputModifiers as validateModifiers,
  stripEmpty,
  DEFAULT_OUTPUT_MODIFIERS,
  DEFAULT_LIST_FILTERS,
  type PipelineInput,
} from '../../../support/helpers/output-pipeline.js';
...
describeFeature(feature, ({ Background, Rule }) => {
  ...
  Rule('List filters compose via AND logic', ({ RuleScenario }) => {
    ...
    RuleScenario('Filter by status and role narrows results', ({ Given, When, Then }) => {
      Given(
        'a dataset with active patterns in roles {string} and {string}',
        (_ctx: unknown, role1: string, role2: string) => {
          state = initState();
          const patterns = [
            createTestPattern({
              name: 'CoreOne',
      ...
      When(
        'I apply list filters with status {string} and role {string}',
        (_ctx: unknown, status: string, role: string) => {
          state!.output = applyListFilters(state!.dataset!, {
            ...DEFAULT_LIST_FILTERS,
            status,
            role,
      ...
      Then('only core patterns are returned', () => {
        ...
        for (const p of results) {
          expect((p.role ?? 'uncategorized').toLowerCase()).toBe('core');
        }
      });
    });

    // Wave 1 retired the `infrastructure` role alias along with the `infra`
    // and `core` legacy values; the matching scenario was removed from the
    // feature file. The `applyListFilters` role-alias path remains exercised
    // through other Wave 1 role values (codec, contract, etc.) elsewhere.
