 ...
 * validation summary counts, pipeline timing, and the
 * frozen QuerySuccess envelope contract.
 *
 * @architect
...
function parseResponseEnvelope(stdout: string): ResponseEnvelope {
  return JSON.parse(stdout) as ResponseEnvelope;
}

function parseResponseMetadata(stdout: string): ResponseMetadata {
  const parsed = parseResponseEnvelope(stdout);
  if (!parsed.metadata) {
  ...
  Rule('Response metadata includes validation summary', ({ RuleScenario }) => {
    RuleScenario('Validation summary in response metadata', ({ Given, When, Then, And }) => {
      ...
      And('stdout is valid JSON with key {string}', (_ctx: unknown, key: string) => {
        const parsed = parseResponseEnvelope(getResult(state).stdout) as Record<string, unknown>;
    ...
    RuleScenario(
      'QuerySuccess envelope preserves the structured JSON contract',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });
        ...
        And('response has success true', () => {
          const parsed = parseResponseEnvelope(getResult(state).stdout);
          expect(parsed.success).toBe(true);
        });

        And('metadata has an ISO timestamp field', () => {
          const metadata = parseResponseMetadata(getResult(state).stdout);
          expect(typeof metadata.timestamp).toBe('string');
          expect(Number.isNaN(Date.parse(metadata.timestamp!))).toBe(false);
        });

        And('metadata has a numeric patternCount field', () => {
          const metadata = parseResponseMetadata(getResult(state).stdout);
