...
describeFeature(feature, ({ Background, Rule, AfterEachScenario }) => {
  ...
  Rule('CLI requires input flag for subcommands', ({ RuleScenario }) => {
    ...
    RuleScenario('Handoff rejects too many modified-file flags', ({ Given, When, Then, And }) => {
      Given('TypeScript files with pattern annotations', async () => {
        await writePatternFiles(state);
      });

      When(
        'I run handoff for {string} with too many modified-file flags',
        async (_ctx: unknown, patternName: string) => {
          const modifiedFiles = Array.from({ length: 201 }, (_value, index) => {
            return `--modified-file src/file-${String(index)}.ts`;
          }).join(' ');
    ...
    RuleScenario(
      'Handoff accepts positional pattern with modified file',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });
    ...
    RuleScenario(
      'Scope-validate rejects conflicting scope values',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });
  ...
  Rule('CLI pattern subcommand shows pattern detail', ({ RuleScenario }) => {
    ...
    RuleScenario(
      'Broken feature-backed pattern reports parser attribution',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });

        And('a broken feature spec for BrokenSpecPattern', async () => {
          await writeTempFile(
            state!.tempContext!.tempDir,
            'features/broken-spec-pattern.feature',
            [
              '@architect',
        ...
        And('output contains parse attribution for {string}', (_ctx: unknown, filePath: string) => {
          const combined = getResult(state).stdout + getResult(state).stderr;
          expect(combined).toContain('spec-parse-failed');
  ...
  Rule('CLI arch subcommand queries architecture', ({ RuleScenario }) => {
    ...
    RuleScenario(
      'Arch bounded-context filters to bounded context',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with architecture annotations', async () => {
          await writeArchPatternFiles(state);
        });
  ...
  Rule('CLI handles argument edge cases', ({ RuleScenario }) => {
    ...
    RuleScenario(
      'Legacy category filter is rejected with role guidance',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });
