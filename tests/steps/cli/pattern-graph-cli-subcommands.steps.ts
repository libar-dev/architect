...
import {
  ...
  runCLICommand,
  writePatternFiles,
  writeCandidateAndDeliveryPatternFiles,
  writeDiagnosticFeatureFiles,
...
describeFeature(feature, ({ Background, Rule, AfterEachScenario }) => {
  ...
  Rule('CLI list subcommand filters patterns', ({ RuleScenario }) => {
    ...
    RuleScenario('List filters candidate status', ({ Given, When, Then, And }) => {
    ...
    RuleScenario('List with removed maturity flag shows error', ({ Given, When, Then, And }) => {
      Given('TypeScript files with pattern annotations', async () => {
        await writePatternFiles(state);
      });
  ...
  Rule('CLI diagnostics subcommand returns extraction diagnostics', ({ RuleScenario }) => {
    RuleScenario(
      'Diagnostics returns extraction failures from feature files',
      ({ Given, And, When, Then }) => {
        Given('TypeScript files with pattern annotations', async () => {
          await writePatternFiles(state);
        });

        And('feature files with extraction diagnostics', async () => {
          await writeDiagnosticFeatureFiles(state);
        });
  ...
  Rule('CLI tags, taxonomy, and sources subcommands return JSON', ({ RuleScenario }) => {
    ...
    RuleScenario('Taxonomy returns taxonomy digest', ({ Given, When, Then, And }) => {
      Given('TypeScript files with pattern annotations', async () => {
        await writePatternFiles(state);
      });
    ...
    RuleScenario('Taxonomy count returns compact text', ({ Given, When, Then, And }) => {
      Given('TypeScript files with pattern annotations', async () => {
        await writePatternFiles(state);
      });
    ...
    RuleScenario('Taxonomy JSON count returns four numeric keys', ({ Given, When, Then, And }) => {
      Given('TypeScript files with pattern annotations', async () => {
        await writePatternFiles(state);
      });
      ...
      And('stdout is a taxonomy count JSON object', () => {
        const parsed = JSON.parse(getResult(state).stdout) as Record<string, unknown>;
        expect(Object.keys(parsed)).toEqual(['roles', 'metadata', 'aggregation', 'total']);
  ...
  Rule('CLI extended arch subcommands query architecture relationships', ({ RuleScenario }) => {
    ...
    RuleScenario(
      'Arch compare returns bounded-context comparison',
      ({ Given, When, Then, And }) => {
        Given('TypeScript files with two bounded contexts', async () => {
