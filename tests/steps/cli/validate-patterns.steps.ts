...
function createTypeScriptPatternFile(patternName: string, _phase: number, status: string): string {
  return `/** @architect */
...
function createGherkinPatternFile(patternName: string, _phase: number, status: string): string {
...
describeFeature(feature, ({ Background, Rule, AfterEachScenario }) => {
  ...
  Rule('Strict mode treats warnings as errors', ({ RuleScenario }) => {
    // Wave 1 reshaped extraction-diagnostic warnings (e.g. invalid `core`
    // role -> warning rather than error); the strict-exit scenario was
    // removed from the feature pending wiring against a current Wave 1
    // warning class. The non-strict path is still exercised below.
