...
describeFeature(feature, ({ Background, AfterEachScenario, Rule }) => {
  ...
  Rule('isDocError type guard classifies errors correctly', ({ RuleScenario }) => {
    RuleScenario('isDocError detects valid DocError instances', ({ Given, When, Then }) => {
      Given(
    ...
    RuleScenario('isDocError rejects non-DocError objects', ({ Given, When, Then }) => {
    ...
    RuleScenario('isDocError rejects null and undefined', ({ Given, When, Then }) => {
      Given('a null value', () => {
  ...
  Rule('formatDocError produces structured human-readable output', ({ RuleScenario }) => {
    RuleScenario('formatDocError includes structured context', ({ Given, When, Then, And }) => {
      Given(
        'a DocError of type "FILE_PARSE_ERROR" with:',
  ...
  Rule('Gherkin extractor collects errors without console side effects', ({ RuleScenario }) => {
    RuleScenario('Errors include structured context', ({ Given, When, Then, And }) => {
    ...
    RuleScenario('No console.warn bypasses error collection', ({ Given, When, Then, And }) => {
  ...
  Rule('CLI error handler formats unknown errors gracefully', ({ RuleScenario }) => {
    RuleScenario('handleCliError formats unknown errors', ({ Given, When, Then }) => {
