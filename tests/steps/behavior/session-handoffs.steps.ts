...
describeFeature(feature, ({ AfterEachScenario, Rule }) => {
  ...
  Rule('Handoff context generation captures session state', ({ RuleScenario }) => {
    RuleScenario(
      ...
      ({ Given, When, Then, And }) => {
        ...
        Then(
          'the output should include {string} section',
    ...
    RuleScenario('Paused phase shows status indicator', ({ Given, And, When, Then }) => {
  ...
  Rule('Handoff templates and checklists contain required sections', ({ RuleScenario }) => {
    RuleScenario('Handoff template exists and contains required sections', ({ Given, Then }) => {
  ...
  Rule('PROCESS_SETUP.md documents handoff and coordination protocols', ({ RuleScenario }) => {
    RuleScenario('PROCESS_SETUP.md documents handoff protocol', ({ Given, Then, And }) => {
  ...
  Rule('Edge cases and acceptance criteria ensure robustness', ({ RuleScenario }) => {
    RuleScenario('Fresh phase shows no previous context message', ({ Given, When, Then, And }) => {
      ...
      Then(
        'the handoff context should show {string} message',
        (_ctx: unknown, _message: string) => {
          ...
          expect(
            docText.includes('no findings') || state!.findingsDocument!.sections.length > 0
          ).toBe(true);
        }
      );
    ...
    RuleScenario('Handoff context can be disabled', ({ Given, When, Then }) => {
    ...
    RuleScenario('Mid-phase handoff preserves context', ({ Given, When, Then, And }) => {
    ...
    RuleScenario('Multiple developers can coordinate', ({ Given, When, Then, And }) => {
    ...
    RuleScenario('Session retrospective captures learnings', ({ Given, When, Then, And }) => {
