...
describeFeature(feature, ({ AfterEachScenario, Rule }) => {
  ...
  // ===========================================================================
  // Behavior File Verification (Traceability) + Inference
  // ===========================================================================

  Rule('Behavior files are verified during pattern extraction', ({ RuleScenario }) => {
    RuleScenario(
    ...
    RuleScenario('Explicit behavior file tag skips verification', ({ Given, When, Then, And }) => {
    ...
    ({ RuleScenario }) => {
      RuleScenario(
        ...
        ({ Given, When, Then, And }) => {
          ...
          Then(
            'the coverage statistics show {int} covered phases',
  ...
  // ===========================================================================
  // Human-Readable Display Names + Explicit Title Override
  // ===========================================================================

  Rule('Pattern names are transformed to human-readable display names', ({ RuleScenario }) => {
    RuleScenario('CamelCase pattern names transformed to title case', ({ Given, When, Then }) => {
    ...
    RuleScenario('PascalCase with consecutive caps handled correctly', ({ Given, When, Then }) => {
      Given('a pattern with patternName {string}', (_ctx: unknown, patternName: string) => {
        state = initState();
        state.inputPatternName = patternName;
        state.pattern = createTestPattern({
    ...
    RuleScenario('Falls back to name when no patternName', ({ Given, When, Then }) => {
      Given(
        'a pattern without patternName but with name {string}',
  ...
  // ===========================================================================
  // PRD Acceptance Criteria Formatting + Full PRD Descriptions
  // ===========================================================================
  ...
  Rule(
    ...
    ({ RuleScenario }) => {
      ...
      RuleScenario('PRD respects includeScenarioSteps flag', ({ Given, When, Then, But }) => {
  ...
  // ===========================================================================
  // Business Value Formatting + Sentence Detection
  // ===========================================================================

  Rule('Business values are formatted for human readability', ({ RuleScenario }) => {
    RuleScenario('Hyphenated business value converted to spaces', ({ Given, When, Then }) => {
    ...
    RuleScenario(
      ...
      ({ Given, When, Then, And }) => {
        Given(
          'roadmap patterns with business values:',
          (_ctx: unknown, dataTable: DataTableRow[]) => {
            state = initState();
            const patterns = dataTable.map((row) => {
              const phase = parseInt(row.Phase ?? '0', 10);
        ...
        And(
          ...
          (_ctx: unknown, dataTable: DataTableRow[]) => {
            ...
            for (const row of dataTable) {
              ...
              expect(found, `Expected business value "${expected}" to be in detail files`).toBe(
                true
              );
            }
          }
    ...
    RuleScenario('File extensions not treated as sentence endings', ({ Given, When, Then }) => {
