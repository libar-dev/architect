...
Feature: lint-patterns CLI
  ...
  Rule: Lint detects violations in incomplete patterns
    ...
    **Verified by:** Report violations for incomplete annotations
    ...
    @validation @tier-a
    Scenario: Reject unresolved uses by default
      Given a TypeScript file "src/unresolved-uses.ts" with unresolved uses
      When running "lint-patterns -i src/unresolved-uses.ts"
      Then exit code is 1
      And stdout contains "missing-relationship-target"

    @validation @tier-a
    Scenario: Reject invalid pattern names by default
      Given a TypeScript file "src/invalid-pattern-name.ts" with invalid pattern name
      When running "lint-patterns -i src/invalid-pattern-name.ts"
      Then exit code is 1
      And stdout contains "invalid-pattern-name"

    @validation @tier-a
    Scenario: Reject dangling pattern removal
      Given the legacy taxonomy fixture "dangling-pattern-removal.ts" is copied to "src/dangling-pattern-removal.ts"
      When running "lint-patterns -i src/dangling-pattern-removal.ts"
      Then exit code is 1
      And stdout contains "RemovedPattern"

    @validation @tier-a
    Scenario: Baseline does not hide new touched-file violations
      Given the legacy taxonomy fixture "touched-file-new-violation.ts" is copied to "src/touched-file-new-violation.ts"
      When running "lint-patterns -i src/touched-file-new-violation.ts"
      Then exit code is 1
      And stdout contains "NotInBaseline"
