  ...
  **Solution:**
  ...
  Background:
    Given a temporary working directory
      ...
      | Per-subcommand help contract        | complete | Yes   | packages/architect-cli/src/cli/pattern-graph-cli.ts      |
      | Public command and flag inventory   | complete | Yes   | packages/architect/tests/features/cli/data-api-help.feature |
      | Structured JSON format compatibility | complete | Yes   | packages/architect/tests/steps/cli/data-api-help.steps.ts |
  ...
  Rule: Per-subcommand help shows usage and flags
    ...
    **Rationale:** Per-subcommand help replaces the need to scroll through full --help output and provides contextual guidance for subcommand-specific flags like --session.
    ...
    Scenario: Global help lists the frozen public command and flag inventory
      When running "pattern-graph-cli --help"
      Then exit code is 0
    ...
    @acceptance-criteria @contract
    Scenario: Structured subcommands accept the public --format json flag
      Given TypeScript files with pattern annotations
      And TypeScript files with architecture annotations and dependencies
      When running the frozen "--format json" contract command set
