...
export function createTsFileWithDirective(options: {
  ...
  // Wave 1 folded @architect-depends-on into @architect-uses; the test
  // option name remains for backward-compat but emits as `uses` in the
  ...
  if (archRole) {
    lines.push(` * @architect-role:${archRole}`);
  }
  if (archContext) {
    lines.push(` * @architect-bounded-context:${archContext}`);
  }
  if (archLayer) {
    lines.push(` * @architect-layer:${archLayer}`);
  }
