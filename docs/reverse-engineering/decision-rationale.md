...
- **CI doctrine is now visible in-tree.** `.github/workflows/ci.yml` and `.github/workflows/publish.yml` are present in the current tree, so the earlier reverse-engineering claim that workflows were absent has gone stale. The remaining trade-off is documentation drift: generated reverse-engineering notes can lag behind the live repo state.
