...
The tag prefix and role registry are configurable via `defineConfig()` (see [Configuration Architecture](#configuration-architecture)).
...
```typescript
// architect.config.ts
import { DDD_ES_CQRS_ROLES, defineConfig } from '@libar-dev/architect/config';
...
| **Extractor**   | `registry.roles`                 | Maps tags to role names                     |
| **Transformer** | `registry`                       | Builds PatternGraph with role indexes       |
...
│    - Select role set (or use default)    │
│    - Apply tagPrefix/fileOptInTag/roles  │
...
| `src/config/role-constants.ts`        | DEFAULT_ROLES, DDD_ES_CQRS_ROLES, RoleDefinition           |
...
Individual declarations are tagged with `@architect-shape` in their JSDoc. The
extractor walks scanned files and pulls every tagged declaration into the
...
```typescript
function buildPatternGraph(options: PipelineOptions): Promise<Result<BuildResult, PipelineError>>;
```
...
| Field          | Type                              | Description                                |
| -------------- | --------------------------------- | ------------------------------------------ |
| `graph`        | `RuntimePatternGraph`             | The fully-computed read model              |
...
| `diagnostics`  | `readonly ExtractionDiagnostic[]` | Structured extraction diagnostics          |
...
interface PatternGraph {
  ...
  relationshipIndex?: Record<
    string,
    {
      ...
      uses: string[]; // declared @architect-uses edges
      ...
      // Derived lookups (computed by transformer)
...
export function transformToPatternGraph(raw: RawDataset): RuntimePatternGraph {
  ...
  const byRoleMap = new Map<string, ExtractedPattern[]>();
  ...
  for (const pattern of patterns) {
    ...
    // Role grouping
    byRoleMap.get(pattern.role)?.push(pattern) ?? /* ... */;
  ...
  return { patterns, tagRegistry, byStatus, byPhase, byQuarter, byRole, bySourceType, counts, /* ... */ };
}
```
...
| -------------------------- | ----------------------------------------------------------------------------- |
| **PatternGraph**           | Aggregated view of all extracted patterns with indexes by role, phase, status |
...
**Purpose:** Taxonomy reference documentation with tag definitions, format type reference, and architecture diagram support.
...
1. **Decision-linked content** — Extracted from retained rules, scenarios, shapes, and decision-linked prose
...
| `behaviorCategories` | `string[]`        | Behavior categories to include in the composed document |
 ...
 * @architect-role service // Implementation classification
...
Individual declarations are tagged with `@architect-shape` in their JSDoc:
...
```gherkin
@architect-pattern:MyPatternExecutableTests @architect-status:roadmap
...
| `@architect-bounded-context:*` | `boundedContext`       |
| `@architect-implements:*`      | `implementsPatterns`   |
| `@architect-uses:*`            | `uses`                 |
...
│  │                                                                              ││
│  │         Computes: byStatus, byPhase, byQuarter, byRole, bySourceType,       ││
│  │                   counts, phaseCount, roleCount, relationshipIndex          ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
│                                        │                                         │
│                                        ▼                                         │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
...
- [CONFIGURATION.md](./CONFIGURATION.md) - Configuration guide, role sets, customization
