...
> doc so type definitions stay synchronized with source evolution.

`transformToMasterDataset()` iterates patterns exactly once, accumulating all
views in a single pass (O(n)). The `byStatus`, `byPhase`, `byQuarter`,
`byCategory`, `bySource`, `counts`, and `relationshipIndex` fields are all
populated in that single loop; codecs and validators read precomputed views
rather than re-filtering raw arrays.
