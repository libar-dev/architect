...
- **`resolveCanonicalRole`** — defined separately at `doc-extractor.ts:71`, `gherkin-extractor.ts:118`, `scanner/gherkin-ast-parser.ts:68`, and a fourth on the _read-side_ in `read-api/pattern-helpers.ts:137`. Four parallel implementations of the same canonicalization rule.
