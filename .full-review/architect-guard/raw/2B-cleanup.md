...
1. Replace 17 wildcards with **8 explicit named exports** matching the 9 consumers (the 4 `run*Cli` are already named-export). The barrel becomes:
   ```ts
   export { runLintPatternsCli, runLintProcessCli, runLintStepsCli, runValidatePatternsCli } from './cli/index.js';
   export { DANGLING_BASELINE_SOURCE_PATH, compareDanglingBaseline, writeDanglingBaseline } from './lint/dangling-baseline.js';
   export type { DanglingBaselineComparison, DanglingBaselineEntry } from './lint/dangling-baseline.js';
   ```
