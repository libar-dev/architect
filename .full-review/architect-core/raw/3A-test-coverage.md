...
- Test callers: **zero**.
- Recommendation: **Promote to tested.** Add FSM transition scenarios (TC-C-3 above). Do NOT delete. Phase 2 was correct that it has zero non-test callers *within `architect-core`*, but the family-wide scan shows it is consumed by `architect-guard`. This is a cross-package dependency that grep limited to `src/` missed.
