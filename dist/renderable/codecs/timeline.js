 ...
 */
export const DEFAULT_MILESTONES_OPTIONS = {
    ...DEFAULT_BASE_OPTIONS,
    filterQuarters: [],
    includeDeliverables: true,
    includeLinks: true,
 ...
 * ```
 */
export function createRoadmapCodec(options) {
 ...
 * ```
 */
export function createMilestonesCodec(options) {
 ...
 * ```
 */
export function createCurrentWorkCodec(options) {
 ...
 */
function buildRoadmapDocument(dataset, options) {
    const sections = [];
 ...
 */
function buildOverallProgress(dataset) {
 ...
 */
function buildPhaseBreakdown(dataset) {
    const sections = [];
 ...
 */
function buildQuarterlyTimeline(dataset) {
    const sections = [];
 ...
 */
function buildPhaseNavigationTable(dataset, options) {
    const sections = [];
    const sortedPhases = [...dataset.byPhase].sort((a, b) => a.phaseNumber - b.phaseNumber);
 ...
 */
export function getPhaseSlug(phaseNumber, phaseName) {
 ...
 */
function buildPhaseDetailFiles(dataset, options) {
    const files = {};
 ...
 */
function buildPhaseDetailDocument(phase, _options) {
    const sections = [];
    const { phaseNumber, phaseName, patterns, counts } = phase;
    const displayName = phaseName ?? `Phase ${phaseNumber}`;
    // Summary
 ...
 */
function buildPatternDetailList(patterns) {
    const sections = [];
    ...
    for (const pattern of sorted) {
        ...
        if (pattern.dependsOn && pattern.dependsOn.length > 0) {
            sections.push(heading(4, 'Dependencies'), list(pattern.dependsOn.map((d) => `Depends on: ${d}`)));
        }
 ...
 */
function buildCompletedMilestonesDocument(dataset, options) {
    const sections = [];
    ...
    if (options.filterQuarters.length > 0) {
        completedPatterns = completedPatterns.filter((p) => p.quarter !== undefined && options.filterQuarters.includes(p.quarter));
    }
    if (completedPatterns.length === 0) {
        sections.push(heading(2, 'No Completed Milestones'), paragraph('No patterns have been completed yet.'));
 ...
 */
function buildCompletedSummary(dataset, completedPatterns) {
    const completedPhases = dataset.byPhase.filter((p) => p.counts.total > 0 && p.counts.completed === p.counts.total).length;
    return [
        heading(2, 'Summary'),
        table(['Metric', 'Value'], [
 ...
 */
function buildCompletedPhases(dataset) {
    const sections = [];
    // Filter to fully completed phases
    const completedPhases = dataset.byPhase.filter((p) => p.counts.total > 0 && p.counts.completed === p.counts.total);
 ...
 */
function buildQuarterlyNavigationTable(_dataset, completedPatterns, options) {
    const sections = [];
    // Group completed patterns by quarter
    const byQuarter = groupBy(completedPatterns.filter((p) => p.quarter), (p) => p.quarter ?? '');
    if (byQuarter.size === 0) {
        return [];
    }
 ...
 */
function buildRecentCompletions(patterns, limit = 10) {
    const sections = [];
    ...
    if (patterns.length > limit) {
        sections.push(paragraph(`Showing ${limit} of ${patterns.length} completed patterns. See quarterly files for full history.`));
    }
    sections.push(separator());
    return sections;
}
 ...
 */
function buildQuarterlyMilestoneFiles(dataset, completedPatterns, _options) {
    const files = {};
    // Group completed patterns by quarter
    const byQuarter = groupBy(completedPatterns.filter((p) => p.quarter), (p) => p.quarter ?? '');
 ...
 */
function buildQuarterDetailDocument(quarter, patterns, dataset) {
    const sections = [];
    // Summary
 ...
 */
function buildCurrentWorkDocument(dataset, options) {
    const sections = [];
    // Get active patterns only
    const activePatterns = dataset.byStatus.active;
    if (activePatterns.length === 0) {
        sections.push(heading(2, 'No Active Work'), paragraph('No patterns are currently in progress.'));
 ...
 */
function buildActivePhases(dataset, options) {
    const sections = [];
 ...
 */
function buildActivePatternsList(patterns, _options) {
    const sorted = sortByPhaseAndName([...patterns]);
 ...
 */
function buildCurrentWorkDetailFiles(dataset, options) {
    const files = {};
 ...
 */
function buildCurrentPhaseDetailDocument(phase, _dataset, options) {
    const sections = [];
    const { phaseNumber, phaseName, patterns, counts } = phase;
    const displayName = phaseName ?? `Phase ${phaseNumber}`;
    // Progress summary
    const progress = completionPercentage(counts);
    const progressBar = renderProgressBar(counts.completed, counts.total, 20);
    sections.push(heading(2, 'Progress'), paragraph(`**Progress:** ${progressBar} (${progress}%)`), table(['Status', 'Count'], [
        ['✅ Completed', String(counts.completed)],
        ['🚧 Active', String(counts.active)],
        ['📋 Planned', String(counts.planned)],
        ['**Total**', String(counts.total)],
 ...
 */
function buildCurrentWorkPatternDetail(pattern, options) {
    const sections = [];
    const name = getDisplayName(pattern);
