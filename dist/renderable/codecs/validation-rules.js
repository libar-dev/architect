...
function buildFSMTransitionsDetailDocument() {
    ...
    for (const [from, targets] of Object.entries(VALID_TRANSITIONS)) {
        ...
        if (targets.length === 0) {
            sections.push(paragraph(`**Terminal state** - no valid transitions. Use \`@libar-docs-unlock-reason\` to modify.`));
        }
        else {
...
function buildErrorCatalogDetailDocument() {
    const sections = [];
    sections.push(heading(2, 'Error Catalog'), paragraph(`Complete error messages and fix instructions for all ${RULE_DEFINITIONS.length} validation rules.`));
    // Summary table
