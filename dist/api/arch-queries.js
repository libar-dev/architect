...
function resolveNeighborEntry(patterns, name) {
...
// computeNeighborhood
// ---------------------------------------------------------------------------
export function computeNeighborhood(name, dataset) {
...
// compareContexts
// ---------------------------------------------------------------------------
function aggregateContextDeps(contextPatterns, relationshipIndex) {
    const deps = new Set();
    for (const p of contextPatterns) {
        const name = getPatternName(p);
        const rels = relationshipIndex?.[name];
        if (rels !== undefined) {
            for (const u of rels.uses)
                deps.add(u);
            for (const d of rels.dependsOn)
                deps.add(d);
        }
    }
    return deps;
}
function findIntegrationPoints(ctx1Patterns, ctx1Name, ctx2PatternNames, ctx2Name, relationshipIndex) {
    const points = [];
...
export function compareContexts(ctx1, ctx2, dataset) {
    ...
    if (ctx1Patterns === undefined || ctx2Patterns === undefined)
        return undefined;
...
// aggregateTagUsage
// ---------------------------------------------------------------------------
export function aggregateTagUsage(dataset) {
    const tagMap = new Map();
    function increment(tag, value) {
    ...
    for (const p of dataset.patterns) {
        ...
        if (p.status !== undefined)
            increment('status', p.status);
        increment('category', String(p.category));
        // Optional enum fields
        if (p.archRole !== undefined)
            increment('arch-role', p.archRole);
        if (p.archContext !== undefined)
            increment('arch-context', p.archContext);
        if (p.archLayer !== undefined)
            increment('arch-layer', p.archLayer);
        // Optional value fields
        if (p.phase !== undefined)
            increment('phase', String(p.phase));
        if (p.priority !== undefined)
            increment('priority', p.priority);
        if (p.quarter !== undefined)
            increment('quarter', p.quarter);
        if (p.team !== undefined)
            increment('team', p.team);
        if (p.effort !== undefined)
            increment('effort', p.effort);
    }
...
// ---------------------------------------------------------------------------
function categorizeFile(filePath, pattern) {
...
function deriveLocationPattern(files) {
    if (files.length === 0)
        return '';
    ...
    for (let i = 0; i < parts.length - 1; i++) {
        if (files.some((f) => f.split('/')[i] !== parts[i]))
            break;
        commonDepth = i + 1;
    }
...
export function buildSourceInventory(dataset) {
