 ...
 */
export function generateTagTaxonomy(registry, config) {
    ...
    sections.push('');
    sections.push('All files must have this tag at the top to be included in documentation extraction:');
    sections.push('');
    ...
    for (const cat of sortedCategories) {
        ...
        const aliases = cat.aliases.length > 0
            ? ` (aliases: ${cat.aliases.map((a) => `\`${registry.tagPrefix}${a}\``).join(', ')})`
            : '';
        sections.push(`| ${cat.priority} | \`${tag}\`${aliases} | ${cat.domain} | ${cat.description} |`);
    }
    sections.push('');
    // Metadata Tags
    ...
    for (const meta of registry.metadataTags) {
        ...
        sections.push(`| \`${tag}\` | ${meta.format} | ${meta.purpose} | ${required} | \`${example}\` |`);
    }
    sections.push('');
    // Aggregation Tags
...
//# sourceMappingURL=tag-taxonomy-generator.js.map