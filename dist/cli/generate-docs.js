...
import { loadArtefactSet, listAvailableArtefactSets, formatArtefactSetError, } from '../config/artefact-set-loader.js';
...
async function main() {
    ...
    let generatorsFromArtefactSet = null;
    if (opts.artefactSet) {
    ...
    const features = opts.features.length > 0 ? opts.features : config?.features ? [config.features] : undefined;
    if (input.length === 0) {
