...
export function camelCaseToTitleCase(text) {
    ...
    for (const acronym of KNOWN_ACRONYMS) {
        ...
        if (result.includes(acronym)) {
            ...
            result = result.replace(new RegExp('([a-z])' + escapedAcronym + '([A-Z])', 'g'), '$1 ' + placeholder + ' $2');
            ...
            result = result.replace(new RegExp('([a-z])' + escapedAcronym + '(?![A-Za-z])', 'g'), '$1 ' + placeholder);
            // 4. Standalone occurrences (start of string, or between non-letters)
            result = result.replace(new RegExp('(?<![A-Za-z])' + escapedAcronym + '(?![A-Za-z])', 'g'), placeholder);
        }
    }
