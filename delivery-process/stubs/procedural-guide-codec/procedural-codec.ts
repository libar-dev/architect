 ...
 * entries to the `referenceDocConfigs` array in `delivery-process.config.ts`.
 ...
 * 3. Two `ReferenceDocConfig` entries in `delivery-process.config.ts`
 * 4. `@libar-docs-include:session-workflows` tag on SessionGuidesModuleSource
 *
 ...
 * Step 4: Update `delivery-process.config.ts` to use `loadPreambleFromMarkdown()` calls
