 ...
 * @libar-docs-used-by MCPServerBin
 * @libar-docs-target src/mcp/server.ts
 * @libar-docs-since DS-MCP
 *
 * ## MCPServer — Entry Point and Lifecycle Manager
 *
 * Main entry point for the delivery-process MCP server. Wires together
 ...
 * via a log() helper that prefixes messages with [dp-mcp].
 *
 ...
 * delivery-process.config.ts using the same applyProjectSourceDefaults() as CLI.
 *
