...
@libar-docs-depends-on:ValidatorReadModelConsolidation
@libar-docs-business-value:separate-cli-shell-from-domain-logic-in-process-api
  ...
  **Solution:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Extract pipeline factory from buildPipeline | pending | src/generators/pipeline/ |
      | Extract handleRules to src/api/rules-query.ts | pending | src/api/rules-query.ts |
      | Extract handleStubs domain logic to stub-resolver | pending | src/api/stub-resolver.ts |
      | Extract handleDecisions domain logic to stub-resolver | pending | src/api/stub-resolver.ts |
      | Extract handlePdr domain logic to stub-resolver | pending | src/api/stub-resolver.ts |
      | Slim process-api.ts to routing shell | pending | src/cli/process-api.ts |
      | validate-patterns.ts consumes pipeline factory | pending | src/cli/validate-patterns.ts |
  ...
  Rule: Pipeline factory is shared across consumers
    ...
    **Verified by:** Pipeline factory is shared, No parallel pipelines
    ...
    @acceptance-criteria
    Scenario: Pipeline factory is shared
    ...
    @acceptance-criteria
    Scenario: No parallel pipelines
  ...
  Rule: Domain logic lives in API modules
    ...
    **Invariant:** Query logic that operates on MasterDataset lives in
    ...
    @acceptance-criteria
    Scenario: Domain logic in API modules
