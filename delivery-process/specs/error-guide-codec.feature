@libar-docs
@libar-docs-pattern:ErrorGuideCodec
...
@libar-docs-business-value:replaces-341-line-manual-PROCESS-GUARD-md-with-auto-generated-error-diagnosis-guides
...
Feature: Error Guide Codec
  ...
  **Design Stubs:**
  ...
  | enhanced-validation-options.ts | delivery-process/stubs/error-guide-codec/ | DD-1: Extended ValidationRulesCodecOptions with includeErrorGuide toggle and EnhancedRuleDefinition interface |
  | error-guide-config.ts | delivery-process/stubs/error-guide-codec/ | DD-2/DD-4: ReferenceDocConfig entry with preamble SectionBlocks and convention tag registration |
  | convention-annotation-example.ts | delivery-process/stubs/error-guide-codec/ | DD-3: Example convention annotation format for all 6 error rules on decider.ts |
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | ReferenceDocConfig entry for PROCESS-GUARD generated reference | complete | delivery-process.config.ts | Yes | integration |
      | Preamble content for Husky/CI setup and programmatic API | complete | delivery-process.config.ts | Yes | integration |
  ...
  Rule: Each error code has fix rationale explaining why the rule exists
    ...
    **Invariant:** Every error code in the generated output includes not just a fix
    ...
    `@libar-docs-convention:process-guard-errors` JSDoc annotations on the error-handling
  ...
  Rule: Convention tags source error context from annotated lint code
    ...
    **Invariant:** Error-handling code in `src/lint/process-guard/` is annotated with
    `@libar-docs-convention:process-guard-errors` using structured JSDoc that includes
