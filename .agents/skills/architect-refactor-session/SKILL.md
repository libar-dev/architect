...
not apply. There is no spec to delete. The **invariant carriers**
...
(ADR-014). For a refactor that means:
...
4. **Edit production code in dependency-leaf-first order.** After each
   ...
   files you changed, then run `pnpm typecheck` at the next phase
...
5. **Update executable Gherkin in lockstep with code.** Every changed
   behavior must appear as a new or edited Scenario; every changed
   invariant must appear in the corresponding Rule block carrying
...
- **Producerless grouping barrels.** When a barrel is only a module
  grouping file and no truthful producer exists, `barrel →
...
- Append a tight per-session entry to
  ...
  drift found and how it was classified (same-root-cause vs
...
- Do not edit `EXECUTION-PLAN.md`, `state.json`, or unstarted
  ...
  Coordinator self-restraint is the load-bearing rule. A
...
- **Half-transferred value.** Code edited but executable Gherkin not
  updated, or vice versa. Code and executable Gherkin must move together. Running
...
If the refactor reveals a missing architectural decision (not just a
