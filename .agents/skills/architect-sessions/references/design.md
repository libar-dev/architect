# Design: plan to design promotion
...
2. **Deliverable files.** Which exact files will this touch? (Becomes the `Background:` table.)
...
5. Re-deriving pattern data outside `PatternGraph`. Read via the graph handle (`pnpm architect:q` over `g.graph` / `g.fsm`). Don't parallel-pipeline.
...
Verify with the graph handle and the typed MCP gates before claiming done:
