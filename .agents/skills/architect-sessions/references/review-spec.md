...
Run the pre-flight from [`../../architect-graph-handle/SKILL.md`](../../architect-graph-handle/SKILL.md) (ADR-014):
...
9. **Overlap with concurrent specs.** Two specs in the same phase touching the same files is a sequencing hazard. Report it.
...
- **Rewriting the spec.** Report the gap; let the design author fix it.
...
- Do not paraphrase the spec back as a summary. Report gaps only.
