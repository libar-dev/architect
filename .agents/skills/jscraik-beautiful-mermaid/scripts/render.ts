...
function parseArgs(): Args {
  ...
  const result: Partial<Args> = { theme: "default", allowInstall: false };
...
Options:
  ...
  --allow-install       Allow auto-install of missing dependencies
...
async function ensurePackage(name: string, allowInstall: boolean): Promise<any> {
  ...
  try {
    ...
    if (!allowInstall) {
      ...
      console.error("Install dependencies explicitly or re-run with --allow-install.");
      process.exit(1);
    }
...
async function main(): Promise<void> {
  ...
  const beautifulMermaid = await ensurePackage("beautiful-mermaid", args.allowInstall);
