...
A one-page map of what the six packages *are now* — recorded from code, not from the (disposable) annotations. Per-package detail lives in each `packages/<pkg>/PRD.md`. This view is direction-agnostic: it serves both the loop-closing MVP for this instance and the types-primary / live-HTML greenfield reimagining.
...
## What survives — the irreducible core (same for the MVP loop *and* the greenfield)
