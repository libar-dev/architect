 ...
 */
const DESCRIPTION_TERMINATORS =
  /^\s*(Background:|Scenario:|Scenario Outline:|Rule:|@\S|Examples:|\||Given |When |Then |And |But )/;
...
export function checkDuplicateAndSteps(
  ...
  let currentScenarioAndSteps = new Map<string, number>();
    ...
    if (stepMatch !== null) {
      ...
      if (previousLine !== undefined) {
        violations.push({
          ...
          message: `Duplicate And step text "${stepText}" — first seen at line ${String(previousLine)}. Consolidate into a single step with DataTable`,
          file: filePath,
          line: i + 1,
        });
      } else {
 ...
 */
export function checkDollarInStepText(content: string, filePath: string): readonly LintViolation[] {
  const violations: LintViolation[] = [];
  const lines = content.split('\n');
 ...
 * are not flagged.
 */
export function checkHashInStepText(content: string, filePath: string): readonly LintViolation[] {
  const violations: LintViolation[] = [];
  const lines = content.split('\n');
 ...
 */
export function runFeatureChecks(content: string, filePath: string): readonly LintViolation[] {
  return [
