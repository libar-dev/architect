 ...
 *
 * Only executable test features are scanned — design specs and decision
 * specs in `architect/` are parsed by @cucumber/gherkin for doc generation
 * and are not vitest-cucumber test inputs. Scanning them creates false
 * compatibility pressure on files that serve a different purpose.
 */
const DEFAULT_FEATURE_GLOBS = ['tests/features/**/*.feature'];
...
export interface StepLintOptions {
  ...
  /** Feature file glob patterns (default: executable test features under tests/) */
