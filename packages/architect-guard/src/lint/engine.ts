...
import type { DocDirective } from '@libar-dev/architect-core';
import type { LintViolation, LintSeverity } from '@libar-dev/architect-core';
...
export function formatPretty(summary: LintSummary, options: { quiet?: boolean } = {}): string {
  ...
  for (const result of summary.results) {
    ...
    for (const v of filtered) {
      ...
      lines.push(`  ${String(v.line)}:1  ${severityColor}  ${v.rule}  ${v.message}`);
    }
  ...
  const parts: string[] = [];
  if (summary.errorCount > 0) {
    parts.push(`${String(summary.errorCount)} error${summary.errorCount === 1 ? '' : 's'}`);
  }
  if (summary.warningCount > 0 && options.quiet !== true) {
    parts.push(`${String(summary.warningCount)} warning${summary.warningCount === 1 ? '' : 's'}`);
  }
  if (summary.infoCount > 0 && options.quiet !== true) {
    parts.push(`${String(summary.infoCount)} info`);
  }
  ...
  if (parts.length > 0) {
    ...
    lines.push(`\u2713 No issues found (${String(summary.directivesChecked)} directives checked)`);
  }
