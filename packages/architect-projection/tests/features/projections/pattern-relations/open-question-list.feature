...
Feature: Open question list projection
  ...
  Rule: Open questions are omitted unless real normalized prose exists
    ...
    **Rationale:** Typed MCP and UI consumers need a machine-readable design-gap surface without reparsing raw Gherkin or returning placeholder empty rows; epic-level gating questions (authored under a qualified heading, on the parent itself) must be reachable, not silently dropped.
