/**
 * DocString MediaType Preservation Step Definitions for architect-core
 *
...
function getStep(scenarioIdx: number, stepIdx: number): ParsedStep | undefined {
  ...
  return scenario?.steps[stepIdx - 1] as ParsedStep | undefined;
}
...
function renderDocString(
  ...
  const language = docString.mediaType ?? defaultLanguage;
  return language.length > 0
    ? { type: 'code', language, content: docString.content }
    : { type: 'code', content: docString.content };
}
