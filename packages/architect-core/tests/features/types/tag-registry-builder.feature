...
Feature: Tag Registry Builder
  ...
  annotation taxonomy, providing tag definitions, roles, and format
  ...
  Rule: buildRegistry returns a well-formed TagRegistry
    ...
    **Invariant:** buildRegistry always returns a TagRegistry with version, roles, metadataTags, aggregationTags, formatOptions, tagPrefix, and fileOptInTag properties.
    ...
    **Verified by:** Registry has correct version, Registry has required metadata tags
  ...
  Rule: Metadata tags have correct configuration
    ...
    **Verified by:** Pattern tag is marked as required, Status tag has default value, Transform functions work correctly
    ...
    Scenario: Transform functions work correctly
      When I build the tag registry
      Then the metadata tag "adr" has a transform function
      And applying the "adr" transform to "5" produces "005"
