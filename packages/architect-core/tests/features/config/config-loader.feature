...
Feature: Config Loader
  ...
  **Solution:**
  ...
  stops at repo root (.git marker), and falls back to DEFAULT_ROLES
  (8 locked Wave 1 roles) if no config found.
  ...
  Rule: Config is loaded and validated
    ...
    **Verified by:** Load valid config with default fallback, Load valid minimal config file, Load config with explicit empty roles, Error on config without default export, Error on config with wrong type
    ...
    Scenario: Load valid config with default fallback
      ...
      And loaded registry should have exactly 8 roles
    ...
    @happy-path
    Scenario: Load config with explicit empty roles
      Given a valid config file with explicit empty roles
      When loading config from base directory
      Then config loading should succeed
      And loaded config should NOT be the default
      And loaded registry should have exactly 0 roles
  ...
  Rule: Config errors are formatted for display
    ...
    **Verified by:** Format error with path and message
    ...
    Scenario: Format error with path and message
      ...
      Then formatted error should contain:
        | text            |
        | Config error    |
        | /test/config.ts |
        | Invalid export  |
