...
export const CLI_SCHEMA: CLISchema = {
  ...
  outputModifiers: {
    ...
    description:
      'Composable with `list`, `arch bounded-context`, and pattern-array `query` methods.',
    postNote: [
      'Valid fields for `--fields`: `patternName`, `status`, `role`, `file`, `source`.',
      '',
  ...
  listFilters: {
    ...
    postNote:
      'Legacy policy: removed `--category` filters are rejected explicitly. Use `--role <name>` instead.',
    options: [
      {
        flag: '--status <status>',
        description: 'Filter by accepted status (candidate, roadmap, active, completed, deferred)',
      },
      {
        flag: '--role <name>',
        description: 'Filter by role tag',
      },
      {
  ...
  commandNarratives: [
      ...
      commands: [
        {
          ...
          expectedOutput: [
            '=== PROGRESS ===',
            '318 delivery patterns (224 completed, 47 active, 47 planned) = 70%',
            '12 candidate patterns excluded from delivery progress',
            '',
            '=== ACTIVE PHASES ===',
      ...
      commands: [
        {
          ...
          details:
            '**Output:** `{ counts: { completed, active, planned, candidate, total }, completionPercentage, distribution }`',
        },
        {
          command: 'list',
          description:
            'Filtered pattern listing. Composable with output modifiers and list filters.',
          usageExample: 'pnpm architect:query -- list --status candidate --names-only',
          details:
            'See Output Modifiers and List Filters for all options. Examples: `list --status candidate --count`, `list --role service --fields patternName,status,file`.',
        },
        {
          command: 'search',
          description:
        ...
        {
          command: 'diagnostics',
          description: 'Structured extraction diagnostics from the current pipeline build.',
          usageExample: 'pnpm architect:query -- diagnostics',
          details:
    ...
    // ---- Architecture Queries (9 subcommands) ----
    {
      ...
      description:
        'All architecture queries output JSON. They use retained `@architect-role` and `@architect-bounded-context` annotations.',
      commands: [
        {
          command: 'arch roles',
          description: 'All roles with pattern counts',
          usageExample: 'pnpm architect:query -- arch roles',
        },
        {
          command: 'arch bounded-context',
          description: 'All bounded contexts',
          usageExample: 'pnpm architect:query -- arch bounded-context',
        },
        {
          command: 'arch bounded-context <name>',
          description: 'Patterns in one bounded context',
          usageExample: 'pnpm architect:query -- arch bounded-context scanner',
        },
        {
    ...
    // ---- Metadata & Inventory (4 commands) ----
    {
      ...
      commands: [
        ...
        },
        {
          command: 'query',
          description:
            'Execute any of the 28 query API methods directly by name. This is the escape hatch for methods not exposed as dedicated subcommands.',
