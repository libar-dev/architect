...
- **MCP server** — does *not* go through this package; `architect-mcp` calls the projections directly. This package is the human/agent-CLI surface only.
...
| `arch graph` | deletion-candidate | The graph itself — *this is the raw emission* the others should derive from. |
...
| `documentation` | deletion-candidate | Renders a doc-type bundle for markdown; the *markdown* sink is a minor consumer, the data is in the emission. |
