...
- **Process Api Cli Cache**: PatternGraph caching between CLI invocations: cache hits, mtime invalidation, and --no-cache bypass.
...
- **Process API Layered Extraction**: `pattern-graph-cli.ts` is 1,700 lines containing two remaining architectural violations of ADR-006: 1.
...
- **Pattern Graph API Testing**: Programmatic interface for querying delivery process state.
