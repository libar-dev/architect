...
- Extract pipeline factory from buildPipeline (pending)
- Extract handleRules to src/api/rules-query.ts (pending)
- Extract handleStubs domain logic to stub-resolver (pending)
- Extract handleDecisions domain logic to stub-resolver (pending)
- Extract handlePdr domain logic to stub-resolver (pending)
- Slim process-api.ts to routing shell (pending)
- validate-patterns.ts consumes pipeline factory (pending)
