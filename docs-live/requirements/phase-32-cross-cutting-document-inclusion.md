...
**Purpose:** Detailed requirements for the Cross Cutting Document Inclusion feature
...
_Verified by: Pattern with include tag appears in reference doc, CSV include routes pattern to multiple documents, Include is additive with existing selectors, Pattern without include tag is unaffected_
...
_Verified by: Diagram scope uses include to select patterns, Pattern in diagram and content via same include tag_
...
_Verified by: Shape with include tag appears in reference doc, Shape with both group and include works, Shape without include but with matching group still works_
...
_Verified by: Convention with include tag appears in reference doc, Include works alongside conventionTags_

## Deliverables

- Replace arch-view with include in tag registry (complete)
- Rename archView to include on directive and pattern schemas (complete)
- Includes field on ExtractedShape (complete)
- Rename arch-view extraction to include in parsers (complete)
- Rename and extract include in doc extractor (complete)
- Extract include CSV from declaration JSDoc (complete)
- Rename DiagramScope.archView to include (complete)
- Add includeTags to ReferenceDocConfig with inclusion pass (complete)
- Rename archView in dataset views and transform (complete)
- Update all source file annotations from arch-view to include (complete)
- Update configs: rename archView, add includeTags (complete)
