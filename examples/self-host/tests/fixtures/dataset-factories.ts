...
import { transformToPatternGraph, createDefaultTagRegistry } from '@libar-dev/architect-core';
import {
  createTestPattern,
...
function createPatternsWithStatusDistribution(
  ...
  const getCategory = () => categories[patternIndex % categories.length] ?? 'core';

  // Wave 1 pattern-identifier regex requires PascalCase (no spaces).
...
export type { TestDeliverable } from './pattern-factories.js';

// Re-export TagRegistry factory for tests that need custom registries
export { createDefaultTagRegistry } from '@libar-dev/architect-core';
